...
sed -i '/\/sys\/devices\/system\/cpu\/cpu0\/cpufreq\/scaling_min_freq/d' /tmp/ramdisk/init.mako.rc
sed -i '/\/sys\/devices\/system\/cpu\/cpu1\/cpufreq\/scaling_min_freq/d' /tmp/ramdisk/init.mako.rc
sed -i '/\/sys\/devices\/system\/cpu\/cpu2\/cpufreq\/scaling_min_freq/d' /tmp/ramdisk/init.mako.rc
sed -i '/\/sys\/devices\/system\/cpu\/cpu3\/cpufreq\/scaling_min_freq/d' /tmp/ramdisk/init.mako.rc
#remove ondemand tuneable overrides, use better ones
sed -i 's/write\ \/sys\/devices\/system\/cpu\/cpufreq\/ondemand\/optimal_freq\ 916000/write\ \/sys\/devices\/system\/cpu\/cpufreq\/ondemand\/optimal_freq\ 300000/g' /tmp/ramdisk/init.mako.rc
sed -i 's/write\ \/sys\/devices\/system\/cpu\/cpufreq\/ondemand\/sync_freq\ 916000/write\ \/sys\/devices\/system\/cpu\/cpufreq\/ondemand\/sync_freq\ 300000/g' /tmp/ramdisk/init.mako.rc
...
found=$(find /tmp/ramdisk/init.mako.rc -type f | xargs grep -oh "# Insert Bricked-Kernel modules");
if [ "$found" != '# Insert Bricked-Kernel modules' ]; then
	#add it
	echo "Adding module insertion to ramdisk!";
	sed -i 's/service conn_init/on property:init.svc.conn_init=stopped\n    # Insert Bricked-Kernel modules\n    insmod \/system\/lib\/modules\/wlan.ko\n    insmod \/system\/lib\/modules\/tun.ko\n    insmod \/system\/lib\/modules\/cifs.ko\n    #insmod \/system\/lib\/modules\/scsi_wait_scan.ko\n\nservice conn_init/' /tmp/ramdisk/init.mako.rc
else
